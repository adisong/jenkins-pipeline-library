...
def call(def params = [:]) {
    ...
    pipeline {
        ...
        options {
            ...
            gitlabBuilds(builds: ['checkout','buildArtifacts', 'buildImage', 'deployQA'])
        }
        stages {
            stage ('checkout'){
                steps {
                    script {
                        ...
                        String branch = sh(returnStdout: true, script: "git branch -a --contains ${env.GIT_COMMIT}").trim()
                        if ( branch.contains('release') ) {
                            deployEnv = "qa1"
                        } else if ( branch.contains('hotfix') ) {
                            deployEnv = "qa2"
                        } else {
                            error("Commit ${env.GIT_COMMIT} found on branches: \n${branch}\nis not on any release or hotfix branch")
                        }
                        def tagPrefix = "refs/tags/"
                        dockerImageTag = env.gitlabSourceBranch.minus(tagPrefix)
            ...
            stage('deployQA'){
                ...
                post {
                    failure {
                        updateGitlabCommitStatus name: 'deployQA', state: 'failed'
                    }
                    success {
                        updateGitlabCommitStatus name: 'deployQA', state: 'success'
                    }
                }
            }
        }
